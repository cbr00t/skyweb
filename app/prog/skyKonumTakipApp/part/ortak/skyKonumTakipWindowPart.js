(function() {
	window.SkyKonumTakipWindowPart = class extends window.SubPart {
		constructor(e) {
			e = e || {};
			super(e);
			...
			const autoCompleteData = param.autoCompleteData || {};
		...
		open(e) {
			...
			const {events, opacity, title} = this;
			let {position} = this;
			/*if (!position) {
				position = {
					x: 'center',
		...
		async tazele(e) {
			...
			if (!this.globalButtonEventsSetFlag) {
				wndContent.find(`button:not(#btnYukari):not(#btnAsagi)`).on('click', evt => {
				...
				this.globalButtonEventsSetFlag = true;
			}
		...
		tazeleSonrasi(e) {
			...
			if (elm && elm.length) {
				elm = autoFocus_uiSelector ? wndContent.find(autoFocus_uiSelector) : null;
				if (elm && elm[0].tagName.toLowerCase() == 'input' && elm[0].select)
					elm[0].select();
			}
		}
			...
			if (tamamIslemi) {
				try {
					...
					if (isError && errorText)
						displayMessage( errorText, `@ ${this.title} @`);
				}
			}
			if (result === false)
				return result;
		...
		tamamIstendi_sonIslemler(e) {
			...
			if (paramDegistimi && param && param.kaydet)
				param.kaydet();
		}
		...
		initTextInput(e) {
			...
			const _e = $.extend({}, {
				...
				/*searchMode: 'containsignorecase', minLength: 1,
		...
		getLayout(e) {
			...
			let template = templatesParent.find(`#${partName}.part`);
			if (!template.length)
				template = templatesParent.find(`.${partName}.part`);
			if (!template.length)
				template = templatesParent.contents(`#${partName}.part`);
			if (!template.length)
				template = templatesParent.contents(`.${partName}.part`);
			
			if (template.length && template[0].tagName.toUpperCase() == 'TEMPLATE')
				template = template.contents('div');
