	...
	window.SkyCafeDetayDuzenlePart = class extends window.SkyCafeWindowPart {
		...
		async tazeleDevam(e) {
			...
			btnIkram
				...
				.off('click')
				.on('click', evt =>
					this.tamamIstendi($.extend({}, e, { event: evt, ikram: !det.ikrammi })));
			btnIkram[det.ikrammi ? 'addClass' : 'removeClass'](`ikram`);
			...
			wndContent.find(`#fiyatTam`).jqxNumberInput({
				...
				value: (fiyat || 0) > 0.01 ? (parts[0] || 0) : 0,
			...
			wndContent.find(`#fiyatFra`).jqxNumberInput({
				...
				value: (fiyat || 0) > 0.01 ? (parts[1] || 0) : 0,
				disabled: !fiyatDegisiklikYapilirmi
			});
			if (fiyatDegisiklikYapilirmi) {
		...
		async tamamIstendi_argsDuzenle(e) {
			...
			const det = this.aktifDetay;
			let flag = e.ikram == null ? e.ikrammi : e.ikram;
			if (flag != null) {
				if (det.ikrammi != flag) {
					det.ikrammi = flag;
					if (!flag)
				...
				await det.brutBedelHesapla();
				await det.netBedelHesapla();
			}
			
			det.miktar = Math.max(det.miktar || 0, 1);
			if (!det.ikrammi && (!det.fiyat || det.fiyat <= 0.01))
