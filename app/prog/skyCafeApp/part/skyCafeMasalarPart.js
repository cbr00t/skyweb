(function() {
	window.SkyCafeMasalarPart = class extends window.SkyCafePartBase {
		...
		tazele(e) {
			const {app} = this;
			const promise = app.promise_masalarTazele = new $.Deferred(async p => {
				if (app.syncServerPort) {
					setTimeout(async () => {
						p.resolve(await this.tazeleDevam(e));
						delete app.promise_masalarTazele;
					}, 200);
				}
				else {
					setTimeout(async e => {
						...
						setTimeout(() => _promise.resolve({ isError: true, rc: 'userAbort' }), 300);
						try { await _promise }
						catch (ex) { }
