(function() {
	window.CETKMGirisPart = class extends window.CETSubPart {
		...
		async kaydet(e) {
			...
			if (sonmu) {
				param.sonKM = km;
				param.sonKMGirildimi = true;
			}
			else {
				param.ilkKM = km;
				param.ilkKMGirildimi = true;
				param.sonKMGirildimi = false;
		...
		onKontrolMesaji(e) {
			...
			if (!sonmu && km < sonKM)
				return `(<b>İlk KM</b>) değeri (<b><i>Önceki Son KM</i></b>) değerinden küçük olamaz`;
			if (km < oncekiKM)
				return `(<b>${sonmu ? 'Son' : 'İlk'} KM</b>)</b>) değeri Önceki değerden küçük olamaz`;
