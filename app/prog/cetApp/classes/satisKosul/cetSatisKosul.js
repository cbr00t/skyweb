			...
			if (result === undefined) {
				const ortakSentOlustur = e => {
					...
					let sent = new MQSent({
						...
						sahalar: $.merge(
							$.merge([`'${e.tip || ''}' detTip`, `kos.vioID`, `kos.kod kosulKod`, 'det.kod', 'det.markaKod'], ekSahalar),
							[`kos.iskontoYapilmazmi`, `kos.promosyonYapilmazmi`])
					});
					if (kosulTip) sent.where.degerAta(kosulTip,  `det.kosulTip`);
					if (kosulKodListe) sent.where.inDizi(kosulKodListe, `det.kosulKod`);
				...
				if (!$.isEmptyObject(recs)) {
					if (recs.length == 1) {
						...
						for (const kosulKod of kosulKodListe) {
