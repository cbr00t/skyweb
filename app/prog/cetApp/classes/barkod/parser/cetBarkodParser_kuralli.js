(function() {
	window.CETBarkodParser_Kuralli = class extends window.CETBarkodParser {
		...
		static async kuralFor(e) {
			e = e || {}; const basKod = e.basKod ?? e.kod, barkod = e.barkod;
		...
		static async barkodKurallariBelirle() {
			const baslangicKod2Kural = this.baslangicKod2Kural = {}, ayrisimKurallari = this.ayrisimKurallari = [], dbMgr = sky.app.dbMgr_mf;
			let stm = new MQStm({
				sent: new MQSent({ from: 'mst_BarTarti', where: `kod <> ''`, sahalar: '*' }),
				orderBy: 'kod'
			});
			...
			stm = new MQStm({
				sent: new MQSent({ from: 'mst_BarAyrisim', where: `kod <> ''`, sahalar: '*' }),
				orderBy: 'kod'
			});
		...
		parcaAl(e) {
			...
			if (value.length < (e.bas + e.hane - 1)) return false
