(function() {
	window.CETBarkodParser_Referans = class extends window.CETBarkodParser {
		...
		static get desteklenenEkOzellikTipSet() {
			...
			if (!result) result = this._desteklenenEkOzellikTipSet = asSet(['model', 'renk', 'desen', 'raf', 'lotNo'])
			return result
		}
		static get uygunEkOzellikTipSet() {
			...
			for (const tip of Object.keys(desteklenenEkOzellikTipSet)) {
				if (tip2EkOzellik[tip]) result[tip] = true
			}
			return result
		}

		async parseDevam(e) {
			e = e || {}; let result = await super.parseDevam(e);
		...
		// barkod referans
			...
			for (const tip of Object.keys(uygunEkOzellikTipSet)) {
				...
				if (idSaha) sent.sahalar.add(`ref.${idSaha} ${idSaha}`)
			}
			const stm = new MQStm({ sent, orderBy: ['varsayilanmi DESC'] });
			let rec = await dbMgr.tekilExecuteSelect(stm);
			if (rec) {
				$.extend(this, rec);
				if (await this.shEkBilgileriBelirle(e)) { e.shRec = rec; return true }
			}
		// stok kodu
