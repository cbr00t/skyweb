(function() {
	window.CETSatisKosulKapsam = class extends window.CObject {
		constructor(e) {
			e = e || {};
			super(e);
		...
		static get sabitAttrListe() {
			if (!this._sabitAttrListe) {
				this._sabitAttrListe = [
					'tarih', 'cari', 'cariTip', 'cariBolge', 'cariKosulGrup',
					'plasiyer', 'plasiyerTip', 'plasiyerBolge'
				];
			}
		...
		uygunmu(e) {
			e = e || {};
			const ozelMusteriListesiVarmi = e._ozelMusteriListesiVarmi || (e.kosul || {}).ozelMusteriListesiVarmi;
			const {sabitAttrListe} = this.class;
			let attrListe = sabitAttrListe;
			if (ozelMusteriListesiVarmi)
				attrListe = attrListe.filter(key => key == 'tarih');
