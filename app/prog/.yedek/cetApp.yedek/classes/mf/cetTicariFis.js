(function() {
	window.CETTicariFis = class extends window.CETStokTicariFis {
		...
		static uygunDetaySinif(e) {
			const rec = e.rec || e;
			if (rec.promokod || rec.promoKod)
				return CETPromosyonDetay;
		...
		get eIslemNumaratorTip() { return super.eIslemNumaratorTip }
		static get pifTipi() { return null }
		static get almSat() { return false }
		...
		/*get hesaplanmisTakipBorcArtisi() {
			return this.class.fiiliCikismi
						? this.hesaplanmisRiskArtisi
						: 0;
		}*/
		...
		hostVars(e) {
			...
			$.extend(hv, {
				nakseklikod: this.nakSekliKod || '',
				tahseklikodno: asInteger(this.tahSekliKodNo) || 0,
				dipiskoran: roundToFra(this.dipIskOran, 2) || 0,
				dipiskbedel: bedel(this.dipIskBedel) || 0
			});
		...
		async setValues(e) {
			...
			const {rec} = e;
			$.extend(this, {
				nakSekliKod: rec.nakseklikod || '',
				tahSekliKodNo: asInteger(rec.tahseklikodno) || null,
				dipIskOran: roundToFra(rec.dipiskoran, 2) || 0,
				dipIskBedel: bedel(rec.dipiskbedel) || 0
			});
		...
		async yeniTanimOncesiIslemler(e) {
			...
			if (!this.dipIskOran)
				await this.getCariStdDipIskOran({ tx: e.tx });
		}
		...
		async initBaslikUI_ara(e) {
			...
			if (this.class.satismi && !this.class.iademi && app.param.faturadaTahsilatYapilirmi) {
		...
		gerekirseDipHesapla(e) {
			super.gerekirseDipHesapla(e);

			let {icmal} = this;
			if (!icmal) {
				icmal = this.dipOlustur(e);
				icmal.hesaplandimi = true;
				return icmal;
			}
		...
		dipOlustur(e) {
			...
			return this.icmal = this.class.icmalSinif.fromFis({ fis: this });
		}

		async promosyonHesapla(e) {
			await this.dipHesapla(e);
			const {sonucBedel} = this;
			const orjDetaylar = this.detaylar;

			const tavsiyeProKod2Stok = {};
			const shKod2Bilgi = {};
			const grupKod2StokSet = {};
			const detaylar = [];
			const _detaylar = this.detaylar;
			for (const i in _detaylar) {
				const det = _detaylar[i].deepCopy();
				detaylar.push(det);
			}
			...
			for (const i in detaylar) {
				...
				if (det.class.promosyonmu) {
					const proKod = det.promoKod;
					tavsiyeProKod2Stok[proKod] = shKod;
				}
				else {
					yDetaylar.push(det);
					if (det.promosyonYapilmazmi)
						continue;

					const grupKod = det.grupKod;
					let bilgi = shKod2Bilgi[shKod] = shKod2Bilgi[shKod] ||
										{ topMiktar: 0, grupKod: grupKod, detaylar: [] };
					bilgi.topMiktar = (asFloat(bilgi.topMiktar) || 0) + (asFloat(det.miktar) || 0);
					bilgi.detaylar.push(det);

					let shKodSet = grupKod2StokSet[grupKod] = grupKod2StokSet[grupKod] || {};
					shKodSet[shKod] = true;
				}
			}
			...
			if (!$.isEmptyObject(yDetaylar)) {
				...
				for (let proTip in promosyonYapilari) {
					if ($.isEmptyObject(shKod2Bilgi))
						break;

					const promosyonListe = promosyonYapilari[proTip];
					for (let i in promosyonListe) {
						const pro = promosyonListe[i];
						const tavsiyeStokKod = tavsiyeProKod2Stok[pro.id];
						let result = await pro.promosyonSonucu($.extend({}, e, {
							detaylar: detaylar,
							shKod2Bilgi: shKod2Bilgi, grupKod2StokSet: grupKod2StokSet,
							tavsiyeStokKod: tavsiyeStokKod
						}));
						const proDet = (result || {}).proDet;
						if (proDet) {
							proDet.promoKod = proDet.promoKod || pro.id;
							yDetaylar.push(proDet);
							if (!pro.class.stokSecimlimi)
								dogrudanProDetListe.push(proDet);
						}
					}
				}
			}
			...
			if (e.islem && !e.gecici && !$.isEmptyObject(dogrudanProDetListe)) {
				let yProDetListe = await new Promise(async (resolve, fail) => {
					let _recs = [];
					let seq2ProDet = {};
					dogrudanProDetListe.forEach((det, seq) => {
						_recs.push({
							seq: seq,
							proKod: det.promoKod,
							proAdi: det.promoAdi,
					...
					let part = new CETPromoUrunSecimPart({
						...
						proDetaylar: _recs,
						kaydedince: e => {
							const recs = e.recs.filter(rec => !!rec.proSHKod);
							const _proDetListe = [];
							recs.forEach(rec => {
								const det = seq2ProDet[rec.seq];
								_proDetListe.push(det);
							});
							resolve(_proDetListe);
						},
						geriCallback: e => {
							resolve({ isError: true, rc: 'vazgecYapildi', errorText: 'İşlem kullanıcı tarafından iptal edildi' })
						}
					});
					await part.run();
				}) || [];
				
				if (yProDetListe && yProDetListe.isError)
					throw recs;
		...
		async onKontrol(e) {
			e = e || {};
			const superResult = await super.onKontrol(e);
			...
			if (kod) {
				...
				if (result == null) {
					result = parseInt(await this.dbMgr.tekilDegerExecuteSelect({
						tx: e.tx,
						query: `SELECT COUNT(*) sayi FROM mst_NakliyeSekli WHERE kod = ?`,
						params: [kod]
					}))
				}
				if (!result)
			...
			if (kod) {
				...
				if (result == null) {
					result = parseInt(await this.dbMgr.tekilDegerExecuteSelect({
						tx: e.tx,
						query: `SELECT COUNT(*) sayi FROM mst_TahsilSekli WHERE kodNo = ?`,
						params: [kod]
					}))
				}
				if (!result)
			...
			if (this.class.sevkTarihKullanilirmi) {
				...
				if (tarih && sevkTarih && asDate(tarih).clearTime() > asDate(sevkTarih).clearTime()) {
					throw {
						isError: true,
						rc: 'invalidValue',
						errorText: `<b>${this.class.sevkTarihEtiket} Tarihi</b> , Fiş Tarihi'nden <u>geri olamaz</u>`
					}
				}
			}
		...
		async kaydetOncesiKontrol_bekleyenSiparis(e) {
			const {siparisRefKontrolEdilirmi, siparisMiktarKontrolEdilirmi} = this.class;
			const {dbMgr, mustKod, detaylar} = this;
			const {gecicimi, islem, tx} = e;
			const eskiFis = !asBool(gecicimi) && (islem == 'degistir') ? e.eskiFis : null;
			let stm, sent, recs;
			...
			if (eskiFis) {
				const _detaylar = eskiFis.detaylar;
				for (const i in _detaylar) {
		...
		// belirlenen stoklar icin tablodaki bekleyenler
			...
			stm = this.class.siparisKontrolStm({
				mustKod: mustKod,
				anahHVListe: anahHVListe
			});
...
// detaylar artik fifo yontemine göre vioID sirasinda karsilanabilir
			...
			stm = new MQStm({
				sent: new MQSent({
					...
					where: [
						{ inDizi: Object.keys(sipIDSet), saha: 'bhar.vioID' }
					],
			...
			for (let i = 0; i < recs.length; i++) {
				const rec = recs[i];
				const tahSekliKodNo = asInteger(rec.tahSekliKodNo) || 0;
				const {vioID, odemeGunKod} = rec;
			...
			if (errorStr) {
				throw {
					isError: true, rc: `bekleyenSiparisKontrol`,
					errorText: errorStr
				}
			}
		}
		...
		static siparisKontrolStm(e) {
			...
			if (!$.isEmptyObject(anahHVListe)) {
				...
				for (const i in anahHVListe) {
					const anahHV = anahHVListe[i];
					const _wh = new MQSubWhereClause({ parantezli: true });
					const stokKod = anahHV.shKod || anahHV.stokKod;
		...
		async kaydetOncesiKontrol_iskOran(e) {
			...
			const iskKullanilirmi = satirIskOranSinir > 0 && (app.iskontoArttirilirmi && this.class.dipIskBedelKullanilirmi);

			if (satirIskOranSinir < 100) {
				let maxOrtIskOran = 0;
				let maxOrtIskOranBelirleForOranlar = oranlar => {
					let yuzdeKalan = 100;
					if (oranlar) {
						for (const i in oranlar) {
							const _oran = oranlar[i];
							if (_oran) {
				...
				for (const i in detaylar) {
					...
					if (!det.ozelIskontoVarmi) {
						maxOrtIskOranBelirleForKeys({ det: det, index: i, keys: detaySinif.iskOranKeys });
				...
				let oranlarIcinYap = oranlar => {
					if (oranlar) {
						for (const i in oranlar) {
							let oran = roundToFra(oranlar[i] * yuzdeKalan / 100, 2);
							yuzdeKalan = bedel(yuzdeKalan - oran);
						}
					}
				}
		...
		async kaydetOncesiKontrol_promosyon(e) {
			if (this.class.promosyonKullanilirmi)
				await this.promosyonHesapla(e);
		}
		...
		async kaydetSonrasiIslemler(e) {
			...
			if (this.class.siparisKontrolEdilirmi)
				await this.kaydetSonrasiIslemler_bekleyenSiparis(e);
		}
		...
		async kaydetSonrasiIslemler_bekleyenSiparis(e) {
			...
			const siparisVioID2DusulecekMiktar = {};
			if (yeniFis) {
				const {detaylar} = yeniFis;
				for (const i in detaylar) {
					const det = detaylar[i];
					// const anahStr = det.getAnahtarStr();
					const {siparisVioID2MiktarYapi} = det;
					for (let vioID in siparisVioID2MiktarYapi) {
						vioID = asInteger(vioID);
						const karsilanan = siparisVioID2MiktarYapi[vioID] || 0;
			...
			if ($.isEmptyObject(siparisVioID2DusulecekMiktar))
				return;
			
			const {dbMgr} = this;
			const hasTx = !!e.tx;
			let tx = hasTx ? e.tx : await dbMgr.getTx();
			for (const vioID in siparisVioID2DusulecekMiktar) {
				...
				if (dusulecekMiktar) {
					const upd = new MQIliskiliUpdate({
						from: `data_BekleyenSiparisler`,
						where: [
							{ degerAta: vioID, saha: `vioID` }
						],
						set: [
							`kalanMiktar = kalanMiktar - ${MQSQLOrtak.sqlDegeri(dusulecekMiktar)}`
						]
					});
			...
			// if (!hasTx)
			// 	tx = await dbMgr.getTx();						// commit
		}

		async getSatisKosulYapilari(e) {
			e = e || {};
			let result = await super.getSatisKosulYapilari(e) || {};
			const kapsam = e.kapsam = $.extend(e.kapsam || {}, {
				tarih: this.tarih,
				cari: this.mustKod
			});
			const cariEkBilgi = await this.getCariEkBilgi();
			if (cariEkBilgi != null) {
				const {tipKod, bolgeKod, kosulGrupKod} = cariEkBilgi;
				if (tipKod != null)
					kapsam.cariTip = tipKod;
				if (bolgeKod != null)
					kapsam.cariBolge = bolgeKod;
				if (kosulGrupKod != null)
					kapsam.cariKosulGrup = kosulGrupKod;
			}
			const {plasiyerKod} = this;
			if (plasiyerKod)
				kapsam.plasiyer = plasiyerKod;

			$.extend(result, await CETSatisKosul.tip2KosulYapilari(e));
		...
		async getPromosyonYapilari(e) {
			...
			let result = await super.getPromosyonYapilari(e) || {};
			e.kapsam = $.extend(e.kapsam || {}, { tarih: this.tarih, cari: this.mustKod });
			$.extend(result, await CETPromosyon.tip2ProYapilari(e));
		...
		async getTahsilSekliAdi(e) {
			...
			if (result == null) {
				...
				const cache = caches.tahsilSekliKodNo2Rec = caches.tahsilSekliKodNo2Rec || {};
				const rec = cache[tahSekliKodNo] || {};
				result = rec.tahSekliAdi || rec.aciklama;
				if (result == null) {
					let stm = new MQStm({
						sent: new MQSent({
							from: `${this.class.table} fis`,
							fromIliskiler: [
								{ from: `mst_TahsilSekli tsek`, iliski: `fis.tahseklikodno = tsek.kodNo` }
							],
							sahalar: [
								`fis.tahseklikodno tahSekliKodNo`,
								`tsek.aciklama tahSekliAdi`
							]
						})
					})
					const rec = await this.dbMgr.tekilExecuteSelect({ tx: e.tx, query: stm });
					result = this.tahSekliAdi = (rec || {}).tahSekliAdi;
				}
			}
			return result;
		}
		...
		async getDokumDegeriDict(e) {
			return $.extend(await super.getDokumDegeriDict(e) || {}, {
				...
				tahsilYazi: async e => {
					e = e || {};
					const tahSekliAdi = await this.getTahsilSekliAdi(e);
					if (tahSekliAdi == null)
						return null;
					if (!tahSekliAdi)
						return ``;
					return `${tahSekliAdi || ''} ile ${bedelStr(this.sonucBedel)} TL TAHSİL EDİLMİŞTİR`
				}
			})
		}
		...
		dokumDipSatirlar(e) {
			const {app} = sky;
			if (app.eIslemKullanilirmi && app.eIslemOzelDokummu && this.eIslemTip)
				return this.dokumDipSatirlar_ozelEIslem(e);

			return this.dokumDipSatirlar_normal(e);
		}

		dokumDipSatirlar_normal(e) {
			const etiketSize = e.bedelEtiketUzunluk - 2, veriSize = e.bedelVeriUzunluk;
			const tekCizgi = ''.padEnd(etiketSize + 2, ' ') + ''.padEnd(veriSize, '-');
			const ciftCizgi = ''.padEnd(etiketSize + 2, ' ') + ''.padEnd(veriSize, '=');
			this.gerekirseDipHesapla();
			const {detaylar, icmal} = this, {cokluKdvmi} = icmal, {brut} = icmal; let yuruyenBakiye = brut,  toplamSatirIskBedel = 0;
			for (const det of detaylar) {
				if (!det.silindimi)
					toplamSatirIskBedel += det.toplamIskontoBedel
			}
			toplamSatirIskBedel = bedel(toplamSatirIskBedel);

			const satirlar = [
				ciftCizgi,
				'BRÜT'.padStart(etiketSize) + ': ' + bedelStr(brut).padStart(veriSize)
			];
			if (toplamSatirIskBedel) {
				// yuruyenBakiye -= toplamIskontoBedel;
				satirlar.push('SATIR ISK.'.padStart(etiketSize) + ': ' + bedelStr(toplamSatirIskBedel).padStart(veriSize))
			}
			const {dipIskOran, dipIskBedel} = this, dipIskVarmi = dipIskOran || dipIskBedel;
			if (dipIskVarmi) {
				if (dipIskOran) {
					const oranIskBedel = bedel(brut * dipIskOran / 100);
					yuruyenBakiye -= oranIskBedel;
			...
			for (const oran in oran2MatrahVeKdv) {
				const matrahVeKdv = oran2MatrahVeKdv[oran], matrah = asFloat(matrahVeKdv.matrah) || 0,
				const kdv = asFloat(matrahVeKdv.kdv) || 0; yuruyenBakiye += kdv;
				const oranText = oran.toString();
				if (oran && (cokluKdvmi /* || dipIskVarmi || toplamSatirIskBedel*/))
					satirlar.push(`KDV MAT. %${oranText}`.padStart(etiketSize) + ': ' + bedelStr(matrah).padStart(veriSize))
				satirlar.push(`KDV %${oranText}`.padStart(etiketSize) + ': ' + bedelStr(kdv).padStart(veriSize))
			}
			if (yuvarlamaFarki) {
				yuruyenBakiye += yuvarlamaFarki;
				satirlar.push('HESAP. KDV'.padStart(etiketSize) +': ' + bedelStr(yuvarlamaFarki).padStart(veriSize))
			}
			satirlar.push(ciftCizgi);
			satirlar.push('SONUÇ'.padStart(etiketSize) + ': ' + bedelStr(icmal.sonuc).padStart(veriSize));
			return satirlar
		}
		dokumDipSatirlar_ozelEIslem(e) {
			const {app} = sky, {ruloParam} = app, {fiyatIskontoGosterim, stokGosterim, kdvVar} = ruloParam;
			...
			if (fiyatIskontoGosterim == 'DP' && toplamSatirIskBedel) {
			...
			if (yuvarlamaFarki) {
				yuruyenBakiye += yuvarlamaFarki;
				satirlar.push('Hesaplanan KDV'.padStart(etiketSize) +': ' + bedelStr(yuvarlamaFarki).padStart(veriSize))
			}
	...
	window.CETSevkiyatFis = class extends window.CETTicariFis {
		...
		static get siparisKontrolEdilirmi() {
			return !this.iademi && sky.app[this.fiiliCikismi ? 'depoSevkiyatSiparisKontrolEdilirmi' : 'depoMalKabulSiparisKontrolEdilirmi']
		}
		static get siparisMiktarKontrolEdilirmi() {
			return !this.iademi && sky.app[this.fiiliCikismi ? 'depoSevkiyatSiparisMiktariKontrolEdilirmi' : 'depoMalKabulSiparisMiktariKontrolEdilirmi']
		}
		...
		get hesaplanmisSiparisVioID2MiktarBilgileri() {
			let result = {}; const, {detaylar} = this, katSayi = this.class.fiiliCikismi ? -1 : 1;
			for (const det of detaylar) {
				if (!det.class.promosyonmu) {
		...
		static async matbuuFormDuzenleRuntime_eIslem(e) {
			...
			const {matbuuForm, tip, fis} = e, {formBilgi, normalSahalar, digerSahalar} = matbuuForm;
			const {sayfaBoyutlari, otoYBasiSonu, bedelEtiketUzunluk, bedelVeriUzunluk} = formBilgi;
			const {eIslemTip} = fis, {pifTipi} = fis.class, width = sayfaBoyutlari.x - 1;
			let boslukAttrLength = 1;
			const spacer = () => ' '.repeat(boslukAttrLength++);
			let y = 1, sahalar = [];
			...
			if (await fis.dokumSahaDegeri({ attr: 'isyeri_unvan2' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '[isyeri_unvan2]' })
			if (await fis.dokumSahaDegeri({ attr: 'isyeri_adres1' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '[isyeri_adres1]' })
			if (await fis.dokumSahaDegeri({ attr: 'isyeri_adres2' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '[isyeri_adres2]' })
			...
			if (yoreVarmi || ilVarmi) {
				const expListe = [];
				if (yoreVarmi) expListe.push('[isyeri_yore]')
				if (ilVarmi) expListe.push('[isyeri_il]')
			...
			if (await fis.dokumSahaDegeri({ attr: 'isyeri_tel1' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: `Telefon: [isyeri_tel1]` });
			if (await fis.dokumSahaDegeri({ attr: 'isyeri_email' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: `e-Posta: [isyeri_email]` });
			...
			if (await fis.dokumSahaDegeri({ attr: 'musteriUnvan2' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '<BOLD>[musteriUnvan2]<NORMAL>' })
			if (await fis.dokumSahaDegeri({ attr: 'musteriAdres1' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '[musteriAdres1]' })
			if (await fis.dokumSahaDegeri({ attr: 'musteriAdres2' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '[musteriAdres2]' })
			yoreVarmi = await fis.dokumSahaDegeri({ attr: 'musteriYore' }); ilVarmi = await fis.dokumSahaDegeri({ attr: 'musteriIl' });
			if (yoreVarmi || ilVarmi)
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '[musteriYoreVeIl]' })
			sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: 'Vergi Daire ve No: [musteriVergiDaireVeVKN]' });
			if (await fis.dokumSahaDegeri({ attr: 'musteriTel' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: `Telefon: [musteriTel]` });
			if (await fis.dokumSahaDegeri({ attr: 'musteriEMail' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: `e-Posta: [musteriEMail]` });
			sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '<LINE0> ' });
			y++;
			...
			if (await fis.dokumSahaDegeri({ attr: 'plasiyerText' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '       Plasiyer: <BOLD>[plasiyerText]<NORMAL>' })
			if (await fis.dokumSahaDegeri({ attr: 'tahsilSekliAdi' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: '   Tahsil Şekli: <BOLD>[tahsilSekliAdi]<NORMAL>' })
			y++;
			
			if (await fis.dokumSahaDegeri({ attr: 'efUUID' }))
				sahalar.push({ pos: { x: 1, y: y++ }, genislik: width, attr: 'ETTN: <BOLD>[efUUID]<NORMAL>' })
			y += 2;
			
			for (const i in sahalar) {
				const saha = new CETMatbuuSaha_Aciklama(sahalar[i]);
				if (saha) normalSahalar.Aciklama[saha.attr] = saha
			}
			...
			x = 1; genislik = 0;
			sahalar.push({
				attr: (fiyatIskontoGosterim == 'NT' ? 'netFiyat' : 'fiyat'), pos: { x: (x += genislik + 1), y: 2 },
				genislik: (genislik = 13), alignment: 'r', tip: 'fiyat'
			});
			if (kdvVar)
				sahalar.push({ attr: 'kdvOraniText', pos: { x: (x += genislik + 1), y: 2 }, genislik: (genislik = 4), alignment: 'r' });
			sahalar.push({
				attr: 'kdvBedel', pos: { x: (x += genislik + 1), y: 2 }, genislik: (genislik = 10), alignment: 'r',
				ozelDonusum: e => bedelStr(e.value)
			});
			...
			for (const i in sahalar) {
				const saha = new CETMatbuuSaha_Detay(sahalar[i]);
				if (saha) normalSahalar.Detay[saha.attr] = saha
			}
			...
			let ekNotlarKey, ekNotlarListe = [];
			switch (eIslemTip) {
				case 'E': ekNotlarKey = 'eFatura'; break;
				case 'A': ekNotlarKey = 'eArsiv'; break;
				case 'IR': ekNotlarKey = 'eIrsaliye'; break;
				case 'BL': ekNotlarKey = 'eBelge'; break;
			}
			if (ekNotlarKey && !$.isEmptyObject(ekNotlar = ruloEkNotlar[ekNotlarKey]))
				ekNotlarListe.push(...ekNotlar)
			if (fis.yildizlimi && !$.isEmptyObject(ekNotlar = ruloEkNotlar[ekNotlarKey = 'isaretli']))
				ekNotlarListe.push(...ekNotlar)
			const ekNotlarDigerKeys = ['eIslemGenel', 'tumGenel'];
			for (const ekNotlarKey of ekNotlarDigerKeys) {
				const items = ruloEkNotlar[ekNotlarKey];
				if (!$.isEmptyObject(items)) ekNotlarListe.push(...items)
			}
			if (!$.isEmptyObject(ekNotlarListe)) {
				const _ekNotlarListe = ekNotlarListe; ekNotlarListe = [];
				for (let text of _ekNotlarListe) {
					if (!text) continue
					const lines = text.split('\n');
					for (const i in lines) {
						text = lines[i].padEnd();
						if (!text || text.length <= width) { ekNotlarListe.push(text); continue }
						while (text.length) {
							const part = text.substr(0, width);
							if (!part) break
				...
				sahalar.push({ pos: { x: 1, y: 0 }, genislik: width, attr: spacer() });
				for (const satir of _ekNotlarListe) {
					if (!satir) continue
					sahalar.push({ pos: { x: 1, y: 0 }, genislik: width, attr: satir })
				}
			}
			for (const i in sahalar) {
				const saha = new CETMatbuuSaha_OtoAciklama(sahalar[i]);
	...
	window.CETIrsaliyeFis = class extends window.CETSevkiyatFis {
		...
		// get eIslemNumaratorTip() { return this.eIslemTip == 'IR' ? 'E' : '' }
		get matbuuFormTip() {
			const {app} = sky;
			if (app.eIslemKullanilirmi && this.eIslemTip) {
				let tip2MatbuuForm = ((app._matbuuFormYapilari || {}).tip2MatbuuForm || {});
				return !tip2MatbuuForm || tip2MatbuuForm['e-Irsaliye'] ? 'e-Irsaliye' : CETFaturaFis.matbuuFormTip;
			}
			return 'Irsaliye';
		}
		...
		async eIslemTipDegeriFor(e) {
			...
			return sky.app.eIslemKullanilirmi ? 'IR' : '';
	...
	window.CETSiparisFis = class extends window.CETTicariFis {
		...
		static get sonStokEtkilenirmi() { return this.sonStokKontrolEdilirmi }
