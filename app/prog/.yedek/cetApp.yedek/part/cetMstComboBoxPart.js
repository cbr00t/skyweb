(function() {
	...
	window.CETMstComboBoxPart = class extends window.Part {
		...
		initComboBox(e) {
			...
			comboBox.jqxComboBox('source', da);
			...
			comboBoxWidget.input.on('blur', evt => {
				...
				if (parentPart && !parentPart.isDestroyed && parentPart.onResize)
					parentPart.onResize({ event: event });
			});
		...
		async comboBox_itemSelectedDevam(e) {
			...
			let rec = e.rec;
			let value = e.value;
			const {table, idSaha, adiSaha} = this;
			this.comboBox.jqxComboBox('placeHolder', this.originalPlaceHolder || '');
			this.focusToDefault();
			if (value != null && table && idSaha) {
				...
				*/
				...
				/*}
					...
					rec = (this.comboBoxWidget.getItemByValue('3') || {}).originalItem;
				}*/
			}
