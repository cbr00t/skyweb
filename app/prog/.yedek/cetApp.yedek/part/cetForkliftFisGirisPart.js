(function() {
	window.CETForkliftFisGirisPart = class extends window.CETListeOrtakPart {
		...
		get adimText() {
			return `${((this.fis || {}).class || {}).aciklama || 'Forklift'}`
		}
		...
		async initNavBar(e) {
							...
							if (true) {
								...
								if (!part) {
									part = this.rafPart = new CETMstComboBoxPart({
										...
										placeHolder: `Raf`, sadeceKodmu: true, listedenSecilmezFlag: true,
			...
			const navBarAfterExpandOrCollapseHandler = this.navBarAfterExpandOrCollapseHandler = e => {
				...
				const collapseFlag = e.collapseFlag;
			...
			setTimeout(async () => {
				...
				const {transferTipKod, yerKod, refYerKod} = fis;
				if (!transferTipKod && (yerKod && refYerKod)) {
				...
				const expandIndex =
						transferTipKod || (yerKod && refYerKod)
							? aktifDetay ? 2 : 1
							: 0;
				if (expandIndex)
					...
					setTimeout(async () => {
						if (aktifDetay) {
							if (this.paletPart) {
								...
								setTimeout(async () => {
									await navBarArrowClickHandler({ index: 2, collapseFlag: true });
									setTimeout(async () => {
										await this.paletPart.comboBox_enterIstendi({ value: aktifDetay.shKod || '' });
										setTimeout(() => this.afterInit(e), 100);
									}, 10);
		...
		async listeArgsDuzenle(e) {
			...
			$.extend(e.listeArgs, {
				/* editable: true, */ serverProcessing: false, pageable: true, filterable: false, filterMode: 'default',
		...
		async palet_itemSelected(e) {
			...
			if (barkod) {
				...
				if (barkodBilgi) {
					...
					if (rafKod) {
						...
						if (!sayi) {
							displayMessage(`<u>${barkod}</u> barkodunun içindeki <u class="bold darkred">${rafKod}</u> kodlu Raf Hatalı veya <b>${refYerKod}</b> kodlu Depo için geçersizdir!`, this.app.appText);
							return false;
						}
					}
			...
			if (det) {
				...
				setTimeout(() => {
					...
					// this.navBarArrowClickHandler({ index: rafKod ? 1 : 2, collapseFlag: false });
				}, rafKod ? 100 : 0);
			}
		}
		...
		async raf_itemSelected(e) {
			...
			let rec = (e.rec || {}).originalItem || e.rec;
			if (!rec) {
				const rafKod = e.value;
				if (this.initFlag && rafKod) {
					displayMessage(`<u class="bold darkred">${rafKod}</u> kodlu Raf Hatalı veya <b>${refYerKod}</b> kodlu Depo için geçersizdir!`, this.app.appText);
					return false;
				}
				return;
			}
