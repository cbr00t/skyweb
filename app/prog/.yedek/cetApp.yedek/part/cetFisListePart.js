	...
	window.CETFisListePart = class extends window.CETListeOrtakPart {
		...
		async fisGirisYap(e) {
			...
			let result = await new $.Deferred(async p => {
				...
				if (islem == 'yeni' && app.class.sonStoktanSecimYapilirmi && fisSinif.sonStoktanSecimYapilirmi) {
					(savedProcs || window).showProgress(null, null, 1, true);
				...
				const fisGirisPartPromise = new $.Deferred(async p => {
					...
					setTimeout(() => {
						let uygunmu = fis && !app.geciciFisKullanilmazmi && !fisGirisIslemiBittimi;
						uygunmu = uygunmu && (
					...
					cellsRenderer: (rowIndex, columnIndex, value, rec) => {
						...
						ekleyici({
							...
							valueGetter: rec => (
								...
								(rec.fisSonuc ? `<br/><span class="veri">${bedelStr(rec.fisSonuc)} TL</span>` : ``)
							)
						});
		...
		loadServerData_buildQuery(e) {
			...
			for (const i in filterGroups) {
				...
				for (const j in filters) {
					...
					if (filter.field == `mustunvan`)
						filter.field = `car.unvan`;
				}
			}
		...
		async merkezeBilgiGonder(e) {
			...
			if (fisID) {
				...
				if (asInteger(await dbMgr.tekilDegerExecuteSelect({ tx: e.tx, query: new MQStm({ sent: sent })}))) {
					...
					// displayMessage(`Bu belge, ekranda düzenleme yapıldığı sırada Arkaplanda Merkeze gönderilmiş ve değişiklik yapılamaz!`, `Belge Transfer`);
					return null;
				}
			}
		...
		geciciFisleriSilIstendi(e) {
			...
			/*return new $.Deferred(p => {
				let wnd = displayMessage(
					`<p class="bold darkred">Geçici fişler silinecek...</p><span>Devam edilsin mi?</span>`,
					this.app.appText,
					true,
					{
						EVET: async (dlgUI, btnUI) => {
							dlgUI.jqxWindow('destroy');
							const result = await this.geciciFisleriSil(e);
							p.resolve(result);
						},
						HAYIR: (dlgUI, btnUI) =>
							dlgUI.jqxWindow('destroy')
		...
		async geciciFisleriSil(e) {
			e = e || {}
			showProgress('Geçici Fişler temizleniyor...', null, -1);
		...
		async belgeTransferIstendiDevam_tamamIstendi_belgeTransfer(e) {
			...
			let sent = new MQSent({
				from: fisSinif.table,
				where: [
					{ degerAta: rec.rowid, saha: idSaha },
					`gonderildi <> ''`
				],
				sahalar: ['COUNT(*) sayi']
			});
			if (asInteger(await fisSinif.dbMgr.tekilDegerExecuteSelect({ tx: e.tx, query: new MQStm({ sent: sent })}))) {
				rec.gonderildi = '*';
				displayMessage(`Bu belge, ekranda düzenleme yapıldığı sırada Arkaplanda Merkeze gönderilmiş ve değişiklik yapılamaz!`, `Belge Transfer`);
				return;
			}
