			...
			let result = await new $.Deferred(async p => {
				...
				if (mustKod && islem != 'izle' && musteriKullanilirmi && app.konumTakibiYapilirmi) {
					...
					if (mevcutKonumBilgi) {
						...
						if (!mustKonumBilgi) {
							const {konumLongitude, konumLatitude, konumAccuracy} = mustRec;
							if (konumLongitude && konumLatitude)
								mustKonumBilgi = { longitude: konumLongitude, latitude: konumLatitude, accuracy: konumAccuracy };
						}
						if (!mustKonumBilgi) {
					...
					cellsRenderer: (rowIndex, columnIndex, value, rec) => {
						...
						ekleyici({
							...
							valueGetter: e => {
								...
								return value ? `<br/><span class="veri">${bedelStr(value)}</span>` : ``
							}
						});
			...
			try {
				...
				(this.app.bilgiGonderTableYapilari || []).forEach(tableYapi => {
					...
					if (fisTable && fisTable == fisSinif.table)
						bilgiGonderTableYapilari.push($.extend({ fisIDListe: [fis ? fis.id : rec.rowid] }, tableYapi));
				});
