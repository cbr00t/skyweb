	...
	window.CETRotaListesiPart = class extends window.CETListeOrtakPart {
		...
		async liste_columnsDuzenle(e) {
			...
			if (!app.bakiyeRiskGosterilmezmi) {
				listeColumns.push({
					text: 'İşlem', align: 'left', dataField: null, width: $(window).width() < 450 ? 110 : 150,
					cellsRenderer: (rowIndex, columnIndex, value, rec) => {
			...
			listeColumns.push(...[
				{
					text: ' ', align: 'right', dataField: 'rotaDisimi', width: 38, filterable: false,
					cellsRenderer: (rowIndex, columnIndex, value, rec) => {
		...
		loadServerData_buildQuery(e) {
			...
			const {app, islemGorenlermi} = this;
			...
			let sent = new MQSent({
				...
				sahalar: (e.rowCountOnly
					? `COUNT(*) sayi`
					: [
						`car.rowid`, `car.kod`, `car.unvan`, `car.yore`, `car.ilKod`, `car.ilAdi`, `car.vkn`,
						`car.konTipKod`, `car.konSubeAdi`,
						`car.efatmi`, `car.herGunmu`, `car.rotaDisimi`, `car.rotaDevreDisimi`,
						`car.bakiye`, `car.riskLimiti`, `car.riskli`,
						'car.takipBorcLimiti', 'car.takipBorc'
					  ])
						/*[	'car.rowid', 'car.*' ])*/
			});
			const {rotaDevreDisiGosterilirmi} = app;
		...
		async loadServerData_ekIslemler(e) {
			...
			_recs.forEach(_rec => {
				...
				if (rec) {
					['topKdv', 'fisSonuc', 'kayitSayisi'].forEach(key =>
						rec[key] = (rec[key] || 0) + (_rec[key] || 0))
				}
			});
			
			const {islemGorenlermi} = this;
		...
		initToolbar(e) {
			e = e || {};
			const layout = e.layout || this.layout;
			const chkIslemGorenlermi = this.chkIslemGorenlermi =
				layout.find(`.toolbar-ust #chkIslemGorenlermi`)
