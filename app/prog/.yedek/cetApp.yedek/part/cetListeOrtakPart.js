	...
	window.CETListeOrtakPart = class extends window.CETSubPart {
		...
		get listeDefaultHeight() {
			return typeof this.height == 'number' || typeof this.height == 'string'
						? this.height
						: $(window).height() - this.divListe.offset().top - ($(window).width() < 450 ? 22 : 40)
		}
		...
		async postInitLayout(e) {
			...
			try {
				...
				setTimeout(() => layout.css('opacity', 1), 500);
		...
		initListePopup(e) {
			...
			if (listePopup) {
				...
				listePopup.jqxMenu({
					...
					animationShowDuration: 0, animationHideDuration: 0, width: 300
				});
				liItems.off('click');
				...
				$(divListe, divListe.find('*, tr, td, td *, div, span, div *, span *'))
					...
					.on('contextmenu', evt => {
						evt.preventDefault();
						let pos = divListe.position();
						listePopup.jqxMenu('open', pos.left + 120, pos.top + 230);
					});
			}
		}
		...
		async onResize(e) {
			return await new $.Deferred(p => {
				setTimeout(async () => {
					...
					if (this.isDestroyed || this.isPrefetch || app.activePart != this || !listeWidget) {
						p.resolve();
						return;
					}
					...
					try {
						...
						try { listeWidget.beginUpdate(); listeWidget.endUpdate(); } catch (ex) { }
						...
						if (!$.isEmptyObject(selectedRecs)) {
							this.afterRefreshOnce = e => {
								this.disableEventsDo(() => {
									if (!$.isEmptyObject(selectedRecs)) {
										try {
											for (const i in selectedRecs)
												listeWidget.selectrowbykey(selectedRecs[i].uid);
										}
			...
			for (const i in filterGroups) {
				...
				for (const j in filterGroup.filters) {
					...
					if (typeof value == 'string')
						value = value.replace(/'/g, `''`).replace(/\*/g, `%`).toLocaleUpperCase();
