(function() {
	window.CETLoginExtension = class extends window.LoginExtension {
		...
		async wsLogin_offline(e) {
			...
			let query = `SELECT COUNT(*) sayi FROM mst_Login WHERE loginTipi = ? AND LOWER(user) = ? AND (pass = ? OR LOWER(pass) = ?)`
			let sayi = parseInt(await dbMgr.tekilDegerExecuteSelect({
				tx: tx, query: query,
				params: [sessionInfo.loginTipi, sessionInfo.user.toLowerCase(), sessionInfo.pass, sessionInfo.passMD5.toLowerCase()]
			}));
			if (!sayi)
		...
		async wsGetUserInfo_offline(e) {
			...
			let tx = await dbMgr.readTransaction();
			let query =
				`SELECT * FROM mst_Login WHERE loginTipi = ? AND LOWER(user) = ?` +
				` AND (${e.infoOnly ? '1 = 1 OR ' : ''}pass = ? OR LOWER(pass) = ?)`;
		...
		async dbSaveLogin(e) {
			...
			await dbMgr.transactionDo(async tx => {
				if (e.clear) {
					let query = `DELETE FROM mst_Login`;
					await dbMgr.executeSql({ tx: tx, query: query });
				}
				
				let query = `REPLACE INTO mst_Login (` +
								`loginTipi, user, pass, userText, ozelTipKod, ozelTipAdi,` +
								`dbName, firmaKisaUnvan, digerSubeleriGorebilirmi` +
							`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
				await dbMgr.executeSql({
					tx: tx, query: query,
					params: [
						sessionInfo.loginTipi, sessionInfo.user, sessionInfo.passMD5, sessionInfo.userDesc,
						(sessionInfo.ozelTip || {}).kod, (sessionInfo.ozelTip || {}).aciklama,
						sessionInfo.dbName, sessionInfo.firmaKisaUnvan,
						asBool(sessionInfo.digerSubeleriGorebilirmi) ? 1 : 0
					]
				})
			});
		}
