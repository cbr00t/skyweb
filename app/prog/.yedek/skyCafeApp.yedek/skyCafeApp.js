	...
	window.SkyCafeApp = class extends window.Prog {
		...
		async wsTanimlarAktifBilgilerYuklendiOrtak(e) {
			...
			if (zHesaplasma) {
				...
				if (rec_bitTS) {
					const kapandimi = !!asDate(rec_bitTS.bedel);
					const {zBilgi} = this;
		...
		async wsGetParams(e) {
			lastAjaxObj = $.ajax({
				cache: false, type: 'GET', datatype: defaultOutput,
		...
		async setMasaAktifFisID(e) {
			...
			if (fisID) {
				...
				upd.set.degerAta(fisAktifMasaID || null, `aktifMasaID`);
			...
			if (masaID) {
				const upd = new MQIliskiliUpdate({
					...
					set: [
						{ degerAta: value || null, saha: `aktifFisID` }
					]
				});
		...
		async zHesaplasmaIstendiDevam(e) {
			...
			if (zKapansinmi) {
				await this.zAcKapatIstendiDevam({ flag: false });
			}
			else {
				const _e = {};
				this.wsAktifBilgileriYukle(_e).then(() =>
