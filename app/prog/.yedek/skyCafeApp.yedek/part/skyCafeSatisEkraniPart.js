(function() {
	window.SkyCafeSatisEkraniPart = class extends window.SkyCafePartBase {
		...
		fisDegisti(e) {
			...
			if (!pratikSatismi) {
				if (app.promise_aktifBilgileriYukle || app.promise_verileriGonder) {
					this.setUniqueTimeout({
						key: 'bekleyenVerileriGonder',
						delayMS: 1000,
						args: e,
		...
		async tazele(e) {
			...
			}, $.isEmptyObject(clickIds) ? 150 : 250);
		}
			...
			if ($.isEmptyObject(recs)) {
				...
				if (textFilter) {
					...
					for (const i in parts) {
						...
						if (part) {
							...
							textFilterPartsTRLower.push(part.toLocaleLowerCase());
						}
					}
				}
				if (!$.isEmptyObject(textFilterParts)) {
					filters.push(e => {
						...
						const aciklamaTRLower = aciklama.toLocaleLowerCase();
						let uygunmu = true;
		...
		async ekle(e) {
			...
			setTimeout(() => delete this.waitPromise, 100);
		...
		async ekleDevam(e) {
			...
			if (rec.degiskenFiyatmi || (rec.fiyat && rec.fiyat <= 0.01)) {
				if (rec.fiyat)
					rec.fiyat = 0;
			}
			...
			if (!$.isEmptyObject(parts)) {
				for (const i in parts) {
					...
					sent.where.add(new MQOrClause([
						...
						{ like: `%${part.toLocaleLowerCase()}%`, saha: `LOWER(mst.aciklama)` }
					]));
				}
			}
			let stm = new MQStm({
