(function() {
	window.SkyCafeTahsilatEkraniPart = class extends window.SkyCafeWindowPart {
		...
		tamamIstendi_argsDuzenle(e) {
			...
			if (sonuc < toplam)
				return { isError: true, errorText: `<b>${bedelStr(toplam - sonuc)} TL</b> <u>AÇIK KISIM</u> kaldı` };
			
			$.extend(e.args, {
				tahsilatBilgi: this.tahsilatBilgi,
				toplam: toplam,
				sonuc: sonuc,
				recs: recs
			});
		...
		kalaniYazIstendi(e) {
			const {wndContent, toplam, tahsilatBilgi} = this;
			if (!toplam)
				return;
			...
			const bedelKeys = ['nakit', 'pos', 'yemekCeki'];
			let kalan = toplam;
			let targetKey;
